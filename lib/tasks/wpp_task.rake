namespace :wpp_task do
  ...
  task test_connection: :environment do
    partners = Partner.where(wpp_connected: true).where.not(remote_jid: nil).where('last_callback_receive < ?',
  ...
  task send_connection_fail_notification: :environment do
    partners = Partner.where(wpp_connected: false).where('last_callback_receive < ?', DateTime.now - 3.hours)
