class Api::V1::Partners::PartnerTestBotController < ApiPartnerController
  before_action :set_message, only: %i[read_message]

  def create_bot_message
    @message = @current_partner.partner_test_bot_messages.create(partner_test_bot_message_params)

    if @message
      render json: PartnerTestBotMessageSerializer.new(@message).serialized_json, status: :ok
    else
  ...
  def test_bot_messages
    @messages = @current_partner.partner_test_bot_messages.order(:created_at)
    render json: PartnerTestBotMessageSerializer.new(@messages).serialized_json, status: :ok
  end

  def last_test_bot_message
    @message = @current_partner.partner_test_bot_messages.order(:created_at).last
  ...
  def destroy_all_messages
    @messages = @current_partner.partner_test_bot_messages

    if @messages.destroy_all
      @current_partner.partner_test_bot_lead.update(lead_classification: nil, conversation_summary: nil, lead_score: 0, token_count: 0)
  ...
  def test_bot_lead
    @partner_test_bot_lead = @current_partner.partner_test_bot_lead

    unless @current_partner.active == true
      if @partner_test_bot_lead.nil?
        @partner_test_bot_lead = PartnerTestBotLead.new(partner: @current_partner,
                                                        lead_classification: nil, conversation_summary: nil, lead_score: nil)
      end
      return render json: PartnerTestBotLeadSerializer.new(@partner_test_bot_lead).serialized_json, status: :ok
    end

    messages = @current_partner.partner_test_bot_messages.order(:created_at)

    last_message = @current_partner.partner_test_bot_messages.order(:created_at).last

    historico_conversa = generate_message_history(@current_partner, messages)

    if !@partner_test_bot_lead.nil? && !last_message.nil? && last_message.created_at + 10.minutes > @partner_test_bot_lead.updated_at
      ...
      @partner_test_bot_lead.update(lead_classification:, conversation_summary:, lead_score: lead_score.to_i)
    end

    render json: PartnerTestBotLeadSerializer.new(@partner_test_bot_lead).serialized_json, status: :ok
  end

  private

  def generate_message_history(partner, messages)
  ...
  def gerar_resposta(pergunta, historico_conversa)
    ...
    begin
      ...
      if response != 'Falha em gerar resposta'
        ...
        @current_partner.calculate_usage(token_cost)
  ...
  def partner_test_bot_message_params
    ActiveModelSerializers::Deserialization.jsonapi_parse(params, polymorphic: [:partner_test_bot_message])
  end

  def set_message
    @message = @current_partner.partner_test_bot_messages.find(params[:id])
  end
end
