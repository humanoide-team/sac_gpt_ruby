...
class Api::V1::WebhooksController < ApiController
  ...
  def whatsapp
    ...
    render json: { status: 'OK', current_date: DateTime.now.to_s, params: } if mensagem_ja_processada(pergunta_usuario)
  ...
  def aguardar_e_enviar_resposta(numero_telefone, tempo_espera = 15)
    ...
    sleep(tempo_espera)
    return unless @tempos_recebimento_mensagens.include?(numero_telefone)

    ultimo_tempo = @tempos_recebimento_mensagens[numero_telefone]
    tempo_atual = DateTime.now

    return unless tempo_atual - ultimo_tempo >= Rational(tempo_espera, 86_400)
  ...
  def mensagem_ja_processada(conteudo_mensagem)
    return true if @mensagens_processadas.include?(conteudo_mensagem)
